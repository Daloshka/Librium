...
## 6. VPN и системный прокси

VPN-клиенты на базе Network Extension (Xray, sing-box, Amnezia, WireGuard,
Tailscale и подобные) поднимают интерфейс `utun` и забирают маршрут по
умолчанию. Пока туннель активен, эффективный прокси системы берётся из
настроек, которые передаёт сам туннель, а не из сервиса Wi-Fi. Проверка:

    scutil --proxy          # под VPN словарь пуст, если туннель не задал прокси
    scutil --nwi            # какой интерфейс первичный: utunN означает VPN
    scutil --nc list        # какие VPN-конфигурации подключены

Задать прокси на сетевом сервисе VPN через `networksetup` можно, но macOS
его не применяет: значение сохраняется, `scutil --proxy` остаётся пустым.
Librium при этом работает, его исходящие соединения идут через туннель,
как у любого приложения. Не работает только доставка запросов в прокси.

Варианты:

- Кнопка «Открыть Chrome через прокси» в диалоге «Подключение HTTPS»:
  отдельный профиль Chromium-браузера с флагом `--proxy-server`, который
  не зависит от системных настроек. Вручную:

      open -na "Google Chrome" --args --proxy-server=127.0.0.1:8080 \
        --user-data-dir="$HOME/Library/Application Support/librium-desktop/chrome-profile"

- Firefox: «Ручная настройка прокси» в параметрах сети браузера.
- Командная строка: `curl --proxy` или переменные `HTTPS_PROXY`/`HTTP_PROXY`
  из раздела 4, они не зависят от системного прокси.
- Свой VPN-клиент: туннель может передать системе прокси через
  `NEProxySettings` в `NEPacketTunnelNetworkSettings`, тогда системный
  прокси снова будет действовать и под туннелем.
...
| Под VPN запросы не попадают в Librium | Туннель Network Extension игнорирует системный прокси. Открой Chrome через прокси кнопкой в диалоге HTTPS или задай прокси в браузере, см. раздел 6. |
