...
server.listen(0,'127.0.0.1',async()=>{try{
  ...
  for(const [kind,f] of Object.entries(files)){
    ...
    await new Promise((resolve,reject)=>http.get({host:'127.0.0.1',port:PROXY_PORT,path:url,headers:{host:`127.0.0.1:${server.address().port}`}},r=>{const chunks=[];r.on('data',c=>chunks.push(c));r.on('end',()=>{try{assert.deepEqual(Buffer.concat(chunks),body);resolve();}catch(e){reject(e);}});}).on('error',reject));
