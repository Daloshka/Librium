...
// The filter fields a session holds and an export sends: one allowlist for both.
function checkFilters(s){
  if(!s||typeof s.query!=='string'||s.query.length>4096||!['','GET','POST','PUT','PATCH','DELETE','OPTIONS','HEAD'].includes(s.method)||!['','2','3','4','5','pending'].includes(s.status)||!['','http','ws'].includes(s.type)||!Array.isArray(s.rules)||s.rules.length>32||s.rules.some(r=>!r||typeof r!=='object'||filters.validate(r)||String(r.value).length>4096))throw Error(t('store.badFilters'));
  return s;
}
function validate(value){
  ...
  for(const s of value.sessions){
    ...
    if(![undefined,"id","method","url","status","size","elapsed"].includes(s.sort)||![undefined,"asc","desc"].includes(s.order))throw Error(t('store.badSort'));
    checkFilters(s);
  }
...
module.exports={read,write,validate,checkFilters};
