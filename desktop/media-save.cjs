...
function mediaFile(detail,side){
  if(!['request','response'].includes(side))throw Error('Неверная сторона запроса');
  const payload=detail[side];
  if(!payload.complete||payload.truncated)throw Error('Тело сохранено не целиком. Дождись загрузки или повтори запрос.');
  ...
  else if(encoding&&encoding!=='identity')throw Error('Не поддерживается сжатие: '+encoding);
